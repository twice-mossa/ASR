...
def _extract_query_features(question: str) -> list[str]:
    ...
    for chunk in zh_chunks:
        ...
        # Chinese exact phrase matching is often too strict, so also keep short n-grams.
        for n in (2, 3):
            if len(chunk) <= n:
                continue
            for index in range(len(chunk) - n + 1):
                gram = chunk[index : index + n]
...
def _pick_segment_indexes(segments: list[TranscriptSegment], question: str, limit: int = 4) -> list[int]:
    features = _extract_query_features(question)
    ranked = sorted(
        (
            (_segment_score(segment.text, features, question), index)
...
def _build_citations_from_indexes(
...
def _build_context_segments(segments: list[TranscriptSegment], indexes: list[int]) -> str:
    lines = []
...
def _build_recent_history(records: list[MeetingQARecord]) -> str:
    if not records:
        return "无"
...
def _build_transcript_excerpt(segments: list[TranscriptSegment], indexes: list[int]) -> str:
    if not segments:
        return ""
...
def _fallback_answer(question: str, citations: list[MeetingCitation]) -> MeetingAskResponse:
    if not citations:
    ...
    bullet_lines = [f"{citation.start:.1f}s - {citation.end:.1f}s：{citation.text}" for citation in citations[:3]]
    answer = (
        f"我先根据当前会议里检索到的相关片段回答“{question}”。如果你要更精确的结论，可以先核对下面这些原始片段：\n"
        + "\n".join(bullet_lines)
    )
    ...
    return MeetingAskResponse(
        answer=answer,
        citations=citations,
        reasoning_summary="已结合问题关键词、相邻上下文片段和最近会议内容进行检索。",
    )
        ...
        "messages": [
            {
                ...
                "content": (
                    ...
                    "answer 要直接回答问题；reasoning_summary 用一句话说明主要依据。"
                ),
            },
            {
                "role": "user",
...
async def ask_meeting_question(meeting_id: int, question: str, current_user: UserProfile) -> MeetingAskResponse:
    ...
    with _get_session() as db:
        ...
        if not (meeting.transcript_text or "").strip():
        ...
        indexes = _pick_segment_indexes(segments, normalized_question)
        citations = _build_citations_from_indexes(segments, indexes)
        context_segments = _build_context_segments(segments, indexes[:4])
        transcript_excerpt = _build_transcript_excerpt(segments, indexes)
        recent_history = _build_recent_history(recent_records)
