...
### PR #30 · Parallelize large audio transcription chunks
- **作者**：g3x1n
- **合并时间**：2026-03-23
- **目标分支**：`dev`
- **变更规模**：+121 / -34，涉及 4 个文件
- **核心内容**：
  - 新增 `GROQ_CHUNK_CONCURRENCY` 配置项，控制并发上传分片数（默认 3）
  - 将大音频分片转录由串行改为基于 `asyncio.Semaphore` 的并发执行
  - 保留分片结果的有序合并：并发完成的分片暂存，按序号依次 emit 合并段
  - 出现异常时自动取消所有未完成任务，并 re-raise 错误
  - 新增测试，验证并发峰值 ≥ 2 且分段顺序正确

---

### PR #29 · Add JWT auth and meeting management improvements
- **作者**：Tony-gift
- **合并时间**：2026-03-23
- **目标分支**：`dev`
- **变更规模**：+482 / -57，涉及 16 个文件
- **核心内容**：
  - 将认证 token 签发与校验切换到 JWT，保留旧版 bearer 兼容
  - 后端新增会议搜索、重命名接口；前端侧边栏同步支持搜索与重命名
  - 转录结果支持关键词搜索与时间戳点击跳转回放
  - `.gitignore` 中忽略构建产出的 PPT 文件

---

## 待合并 PR（Open）

### PR #31 · Add Speechmatics speaker diarization
- **作者**：twice-mossa
- **状态**：Open（未合并）
- **创建时间**：2026-03-24
- **变更规模**：+705 / -20，涉及 11 个文件
- **核心内容**：
  - 新增基于 Speechmatics 的说话人分离（diarization）后处理步骤
  - 数据库模型新增说话人标签、置信度、diarization 状态字段，持久化至会议和转录分段
  - 前端转录结果支持按说话人分组展示，标签颜色区分
  - 测试层隔离真实 MySQL 和外部 Speechmatics 调用，使用 mock 验证核心流程
...
| 合并 PR 数 | 2 个（#29、#30） |
| 待合并 PR 数 | 1 个（#31） |
| 主要推进方向 | 大音频并发转录优化、JWT 认证升级、会议管理增强 |
| 本阶段里程碑 | 转录性能提升；认证标准化；说话人分离能力初步落地 |
| 当前待做 | PR #31 合并、RAG 问答接口、更完整的会议管理操作 |
